...
function registerSongsUrl(app) {
  app.get('/api/getSongsUrl', (req, res) => {
    ...
    // 以歌曲的 mid 为 key，存储歌曲 URL（同时容纳自定义和 QQ 音乐的 URL）
    const urlMap = {}
    ...
    if (nonCustomMids.length > 100) {
      const groupLen = Math.ceil(nonCustomMids.length / 100)
      for (let i = 0; i < groupLen; i++) {
        midGroup.push(nonCustomMids.slice(i * 100, (100 * (i + 1))))
      }
    } else {
      midGroup = [nonCustomMids]
    }
  ...
  app.get('/api/getLyric', (req, res) => {
    ...
    get(url, data).then((response) => {
      ...
      if (result.code === ERR_OK) {
        ...
        if (result.lyric) {
          try {
            lyric = Buffer.from(result.lyric, 'base64').toString('utf8')
          } catch (e) {
            lyric = '歌词解码失败'
          }
        }
        ...
        res.json({
          ...
          result: {
            lyric: lyric,
            trans: result.trans ? Buffer.from(result.trans, 'base64').toString('utf8') : ''
          }
        })
      } else {
        res.json(result)
